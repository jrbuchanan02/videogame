...
{
    for ( std::size_t i = 0; i < ( sizeof ( unittests ) / sizeof ( Unittest ) );
          i++ )
    {
        if ( unittests [ i ].test ( ) )
        {
...
bool testSigmaCheck ( )
{
    ...
    int unexpect = 0;
    std::cout << "Rolled 11,000 times, and got these sigma-check pass/fail "
                 "rates:\n";
    for ( int i = 0; i < 11; i++ )
    {
        std::cout << std::fixed;
...
template <class CharT> bool basicSyncstreambufTest ( )
{
    ...
    io::base::basic_syncstreambuf<CharT> buf (
            io::base::get_cout<CharT> ( ).rdbuf ( ) );
...
bool testCodePointSplitting ( )
{
    ...
    auto testTwo = [] ( std::string against, char escape, char with ) {
        ...
        std::cout << "Where \"" << with << "\" represents codepoint u+"
                  << std::hex << ( int ) escape;
        ...
        std::cout << "Since \"" << replace ( result.at ( 0 ) ) << "\" "
                  << conditions [ 0 ];
    ...
    auto testBig = [ & ] ( ) {
        ...
        std::cout << "Testing \""
                  << "[Final Private Use Character Here]"
                  << "\"\n";
        std::cout << "Where [Final Private Use Character Here]"
                  << " represents U+10FFFF\n";
        auto        result = io::console::manip::splitByCodePoint ( against );
        bool        pass   = against.starts_with ( result.at ( 0 ) );
        std::string conditions [ 2 ] = {
