...
namespace io::console::manip
{
     ...
     * @note This algorithm can be classified as "best effort". If it cannot split
     * a sequence of text which occupies the entire line, that text will wrap, 
     * but the function will do its best to keep that one word wrapping to the 
     * lines which contain it.
     ...
     * of columns. This function assumes that we have already wrapped the string.
     ...
     * of the next word.
     * 2. Attempt to stretch a hyphen (- to --), equal-sign (= to ==), or similar
     * character.
     * 3. Attempt to find a known fullwidth character to replace an existing one with
